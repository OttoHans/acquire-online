...
// Then set VAPID_PUBLIC and VAPID_PRIVATE env vars on Railway, or they'll be auto-generated each boot (breaks existing subs)
...
if (!VAPID_PUBLIC || !VAPID_PRIVATE) {
  ...
  console.warn('VAPID keys not set — generated ephemeral keys. Push subs will break on restart. Set VAPID_PUBLIC and VAPID_PRIVATE env vars.');
}
...
async function saveGame(pin, state) {
  await pool.query(
    `INSERT INTO games(pin,state,updated_at) VALUES($1,$2,NOW())
     ON CONFLICT(pin) DO UPDATE SET state=$2, updated_at=NOW()`,
...
async function savePushSub(pin, playerName, sub) {
  await pool.query(
    `INSERT INTO push_subs(pin,player_name,subscription) VALUES($1,$2,$3)
     ON CONFLICT DO NOTHING`,
    [pin, playerName, JSON.stringify(sub)]
  );
}

async function getPushSubs(pin, playerName) {
  const r = await pool.query('SELECT subscription FROM push_subs WHERE pin=$1 AND player_name=$2', [pin, playerName]);
  return r.rows.map(row => row.subscription);
}

async function sendPush(pin, playerName, title, body) {
  const subs = await getPushSubs(pin, playerName);
  for (const sub of subs) {
    try {
      await webpush.sendNotification(sub, JSON.stringify({ title, body, pin }));
    } catch (e) {
      if (e.statusCode === 410) {
        await pool.query('DELETE FROM push_subs WHERE pin=$1 AND player_name=$2', [pin, playerName]);
      }
    }
  }
}
...
// ── GAME CONSTANTS ──
const CHAINS = [
  { id: 'worldwide',   name: 'Worldwide',   tier: 1, color: '#60c46a' },
  { id: 'sackson',     name: 'Sackson',     tier: 1, color: '#e05252' },
  { id: 'festival',    name: 'Festival',    tier: 2, color: '#9d60d4' },
  { id: 'imperial',    name: 'Imperial',    tier: 2, color: '#d45a5a' },
  { id: 'american',    name: 'American',    tier: 2, color: '#e0a040' },
  { id: 'continental', name: 'Continental', tier: 3, color: '#5ad4d4' },
  { id: 'zeta',        name: 'Zeta',        tier: 3, color: '#5aa8e0' },
];
...
function allTiles() {
  const t = [];
  for (const c of COLS) for (const r of ROWS) t.push(c + r);
  return t;
}
...
function neighbors(tid) {
  const col = tid.slice(0, -1), row = tid.slice(-1);
  const ci = COLS.indexOf(col), ri = ROWS.indexOf(row);
  const n = [];
  if (ci > 0) n.push(COLS[ci-1] + row);
  if (ci < COLS.length-1) n.push(COLS[ci+1] + row);
  if (ri > 0) n.push(col + ROWS[ri-1]);
  if (ri < ROWS.length-1) n.push(col + ROWS[ri+1]);
  return n;
}
...
function sharePrice(chainId, size) {
  ...
  const b = [0,200,300,400,500,600,700,800,900,1000,1100,1100,1100,1100,1100,1100,1100,1100,1100,1100,1100,
    1200,1200,1200,1200,1200,1200,1200,1200,1200,1200,
    1300,1300,1300,1300,1300,1300,1300,1300,1300,1300,1400,1500];
  return (b[Math.min(size, b.length-1)] || b[b.length-1]) + (tier-1)*100;
}
function payBonuses(state, chainId, log, pin) {
  const size = chainSize(state.board, chainId);
  ...
  if (holdings.length === 1) {
    ...
    if (majH.length > 1) {
      ...
      if (minH.length) {
        ...
        log(`${minH.map(h=>state.players[h.i].name).join(', ')} minority bonus $${share.toLocaleString()} for ${chain.name}`);
      }
    }
  }
  if (pin && bonusEvents.length) {
    setTimeout(()=>broadcastToRoom(pin, {type:'bonus', bonuses:bonusEvents}), 100);
  }
}
...
function classifyPlaced(board, tid) {
  ...
  if (!chainNb.length) {
    if (neutralNb.length && availableChains(board).length) return { type: 'found', neutralNb };
    return { type: 'lone' };
  }
  if (chainNb.length === 1) return { type: 'grow', chain: chainNb[0], neutralNb };
  if (chainNb.filter(id => isSafe(board, id)).length >= 2) return { type: 'unplayable' };
...
function createGameState(playerNames, mode) {
...
// Map: pin -> Set of { ws, playerName }
const rooms = new Map();
function getRoomClients(pin) {
  if (!rooms.has(pin)) rooms.set(pin, new Set());
  return rooms.get(pin);
}
function broadcastToRoom(pin, msg) {
  const data = JSON.stringify(msg);
  getRoomClients(pin).forEach(({ ws }) => { if (ws.readyState === 1) ws.send(data); });
}
function sendToPlayerInRoom(pin, playerName, msg) {
  getRoomClients(pin).forEach(({ ws, name }) => {
    if (name === playerName && ws.readyState === 1) ws.send(JSON.stringify(msg));
  });
}
function publicState(state) {
  return {
    ...
    pendingMerge: state.pendingMerge ? {
      ...
      waitingFor: state.pendingMerge.waitingFor,
    } : null,
...
function sendPrivateHand(pin, state, playerName) {
  const p = state.players.find(pl => pl.name === playerName);
  if (p) sendToPlayerInRoom(pin, playerName, { type: 'private', hand: p.hand });
}
async function broadcastState(pin, state) {
  broadcastToRoom(pin, { type: 'state', state: publicState(state) });
  getRoomClients(pin).forEach(({ name }) => sendPrivateHand(pin, state, name));
}

// ── LOBBY STATE (in-memory, per pin) ──
const lobbies = new Map(); // pin -> { players:[{name}], open:bool }
...
app.post('/new-game', async (req, res) => {
  ...
  lobbies.set(pin, { players: [], open: true, mode });
  res.json({ pin });
});
...
wss.on('connection', ws => {
  ...
  ws.on('message', async raw => {
    try {
      ...
      if (msg.action === 'start_game') {
        ...
        lobby.open = false;
        const state = createGameState(lobby.players.map(p => p.name), lobby.mode || 'beginner');
  ...
  ws.on('close', () => {
    if (myPin) {
      const clients = getRoomClients(myPin);
...
async function handleGameAction(state, pi, myName, msg, pin) {
  ...
  const notifyCurrentPlayer = async () => {
    ...
    if (cp.name !== myName) {
      await sendPush(pin, cp.name, 'ACQUIRE — Your Turn!', `It's your turn to play.`);
    }
  };
  ...
  if (msg.action === 'place_tile') {
    ...
    if (!chainNb.length && neutralNb.length && availableChains(state.board).length) {
      ...
      absorbed.forEach(chainId => {
        state.players.forEach((p,ppi) => { if (p.stocks[chainId]>0) waitingFor.push({ pi:ppi, chainId, confirmed:false }); });
      });
  ...
  if (msg.action === 'buy_stock') {
    ...
    if (ac.length && (ac.every(c=>isSafe(state.board,c)) || ac.some(c=>chainSize(state.board,c)>=41))) {
      endGame(state, addLog); return true;
    }
...
dbInit().then(() => {
  server.listen(PORT, () => console.log(`Acquire server on port ${PORT}`));
}).catch(e => {
  console.error('DB init failed:', e);
  // Start anyway (works without DB in dev)
